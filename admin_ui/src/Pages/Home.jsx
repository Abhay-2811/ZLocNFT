...
const Home = () => {
  ...
  const [txSealed, setTxSealed] = useState({ bool: false, txID: 0, add: '' })
  ...
  const handleSubmit = async e => {
    ...
    const transaction = await fcl
      ...
      .onceSealed()
      .then(() => {
        setTxSealed({ bool: true, txID: transactionID, add: add })
        setTxSuccess(true)
      })
  ...
  return (
    <>
      {txSealed.bool ? (
        <Popup txID={txSealed.txID} add={txSealed.add} />
      ) : (
        <form className='ml-60 mt-36 mr-20 ' onSubmit={handleSubmit}>
          ...
          <div className='relative z-0 w-full mb-6 group'>
            ...
            <label
              ...
              className='peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6'
            >
              Enter Valid Ticket Numbers ( Each separated by " <b>,</b> "
